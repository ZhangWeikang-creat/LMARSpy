...
class FpsBoundary:
    ...
    def exchange_boundary(fps: FpsData, local_data):
        ...
        if global_npy > 2:
            s_send = cal.contiguous(local_data[:, ng:2*ng, :]).cpu()
            n_send = cal.contiguous(local_data[:, -2*ng:-ng:, :]).cpu()

            s_recv = np.empty(s_send.shape, dtype=dtype)
            n_recv = np.empty(n_send.shape, dtype=dtype)

        if global_npx > 2:
            w_send = cal.contiguous(local_data[:, :, ng:2*ng]).cpu()
            e_send = cal.contiguous(local_data[:, :, -2*ng:-ng]).cpu()
    ...
    def exchange_boundary_create(fps: FpsData, send_data, w_recv, e_recv, s_recv, n_recv, reqs):
        ...
        if global_npy > 2:
            ...
            if py == 1: # 不能自己跟自己传输数据，但MPI可以
                ...
                for i in range(fps.size): # 必须给出传输的顺序，不然通信会锁死， MPI没有这个问题
                    ...
                    if y_start > 0:
                        ...
                        s_rank = i + px*(py-1)
                    if fps.rank == i:
                        reqs.append(dist.isend(tensor=s_send, dst=s_rank, tag=tag))
                    if fps.rank == s_rank:
                        reqs.append(dist.irecv(tensor=n_recv, src=i, tag=tag))
                ...
                for i in range(fps.size):
                    ...
                    if y_end < ny:
                        ...
                        n_rank = i - px*(py-1)
                    if fps.rank == i:
                        reqs.append(dist.isend(tensor=n_send, dst=n_rank, tag=tag))
                    if fps.rank == n_rank:
                        reqs.append(dist.irecv(tensor=s_recv, src=i, tag=tag))
            ...
            if px == 1:
                ...
                for i in range(fps.size):
                    ...
                    if x_start > 0:
                        ...
                        w_rank = i +  px -1
                    if fps.rank == i:
                        reqs.append(dist.isend(tensor=w_send, dst=w_rank, tag=tag))
                    if fps.rank == w_rank:
                        reqs.append(dist.irecv(tensor=e_recv, src=i, tag=tag))
                ...
                for i in range(fps.size):
                    ...
                    if x_end < nx:
                        ...
                        e_rank = i - px + 1
                    if fps.rank == i:
                        reqs.append(dist.isend(tensor=e_send, dst=e_rank, tag=tag))
                    if fps.rank == e_rank:
                        reqs.append(dist.irecv(tensor=w_recv, src=i, tag=tag))
