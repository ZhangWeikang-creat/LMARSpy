...
class DynEulSwLinear:
    ...
    def step(dyn: DynData, dt: float, ng: int = 1) -> None:
        ...
        if mp.npx > 2:
            #--- get fx
            DynUtil.get_var_we(-Const.GRAV*dz, fxw, fxe, mp, ng)
            DynUtil.get_x_upwind(fx, fxw, fxe, ut)
            fx[:] = fx*xfx
        if mp.npy > 2:
            #--- get fy
            DynUtil.get_var_sn(-Const.GRAV*dz, fys, fyn, mp, ng)
        ...
        if mp.npx > 2:
            rhs_ua[:] = 0
            #=== apply pgrad x
            DynUtil.apply_pgrad_x_eul(rhs_ua, prcx, -1/Const.GRAV, dg.a_rdx[jb:je,ib:ie], dt)
            ua[:,jb:je,ib:ie] = ua0[:,jb:je,ib:ie] + rhs_ua
        ...
        if mp.npy > 2:
            rhs_va[:] = 0
            #=== apply pgrad y
            DynUtil.apply_pgrad_y_eul(rhs_va, prcy, -1/Const.GRAV, dg.a_rdy[jb:je,ib:ie], dt)
            va[:,jb:je,ib:ie] = va0[:,jb:je,ib:ie] + rhs_va
