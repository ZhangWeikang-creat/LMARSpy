...
from .reconst_torch import DynRc
...
class DynUtil:
    ...
    def get_xfx(xfx, ut, dt: float, c_dy):
        ...
        xfx[:] = ut * torch.unsqueeze(c_dy, dim=0) * dt
    ...
    def get_yfx(yfx, vt, dt:float, d_dx):
        ...
        yfx[:] = vt * torch.unsqueeze(d_dx, dim=0) * dt
    ...
    def update_eul(rhs_qa, fx, fy, fz, dz, a_rda):
        ...
        rhs_qa[:] += ((fx[:,:,:-1] - fx[:,:,1:]) + (fy[:,:-1,:] - fy[:,1:,:]))\
                    * torch.unsqueeze(a_rda, dim=0) + (fz[:-1,:,:] - fz[1:,:,:]) / dz
    ...
    def apply_pgrad_x_eul(rhs_ua, prc, dz, a_rdx, dt: float):
        ...
        rhs_ua[:] += -grav*dz * (prc[:,:,:-1] - prc[:,:,1:]) * torch.unsqueeze(a_rdx, dim=0) * dt
    ...
    def apply_pgrad_y_eul(rhs_va, prc, dz, a_rdy, dt:float):
        ...
        rhs_va[:] += -grav*dz * (prc[:,:-1,:] - prc[:,1:,:]) * torch.unsqueeze(a_rdy, dim=0) * dt
